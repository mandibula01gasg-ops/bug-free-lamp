...
export function ProductCard({ product, onAddToCart, hasPromo = false, promoText = "APENAS HOJE 40%", buttonText, isPriority = false, discountedFrom }: ProductCardProps) {
  ...
  // Usa originalPrice do produto se existir, senão usa discountedFrom, senão usa currentPrice
  const productOriginalPrice = product.originalPrice ? parseFloat(product.originalPrice) : null;
  const originalPrice = productOriginalPrice || discountedFrom || currentPrice;
  const discountedPrice = hasPromo ? originalPrice * 0.6 : currentPrice;
  const discountPercent = (productOriginalPrice || discountedFrom)
